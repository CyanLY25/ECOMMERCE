#!/usr/bin/env python3
"""
Script para ejecutar el entrenamiento del modelo CNN-GRU.
"""
import sys
...
def main():
    ...
    # Generar secuencias
    X_train_seq, y_train_seq = create_sequences(X_train, y_train, config.CNN_GRU_WINDOW_SIZE)
    X_val_seq, y_val_seq = create_sequences(X_val, y_val, config.CNN_GRU_WINDOW_SIZE)
    X_test_seq, y_test_seq = create_sequences(X_test, y_test, config.CNN_GRU_WINDOW_SIZE)
    
    # Entrenar
    cnn_gru = CNNGRUModel(config)
    history, metrics, training_time = cnn_gru.train(X_train_seq, y_train_seq, X_val_seq, y_val_seq)
    
    # Evaluar en test
    print("\n" + "="*60)
    print("RESULTADOS DEL MODELO CNN-GRU")
    print("="*60)
    y_test_pred = cnn_gru.model.predict(X_test_seq, verbose=0).flatten()
    ...
    expected_files = [
        config.CNN_GRU_MODEL_PATH,
        config.CNN_GRU_HISTORY_PATH,
        config.CNN_GRU_METRICS_PATH,
        config.CNN_GRU_LOG_PATH,
        config.FIGURES_DIR / "cnn_gru_loss.png",
        config.FIGURES_DIR / "cnn_gru_mae.png"
    ]
    ...
    try:
        loaded_model = tf.keras.models.load_model(config.CNN_GRU_MODEL_PATH)
    ...
    save_metrics(final_metrics, config.REPORTS_DIR / "cnn_gru_metrics.json")
